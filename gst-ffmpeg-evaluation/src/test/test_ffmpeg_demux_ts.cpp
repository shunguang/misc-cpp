...
static ngv::AppCpuTimeProf g_cpuProf(100);
...
static int open_decoder(AVCodecContext** dctx, AVFormatContext* fctx) {
...
#include "libUtil/util.h"
using namespace ngv;
int test_ffmpeg_demux_ts(const CfgApp &cfg0 ) 
{
    AppTicToc ttSeq;
    ttSeq.tic();
    ...
    const char *vfile = cfg->inputFilePath.c_str();
   	g_cpuProf.setThdCntToPrintOut(cfg->nFrmsToPeekCpuTimeProfile);

    if (!ngv::fileExists( std::string( vfile ) )){
        fprintf(stderr, "file <%s> does not exist\n", vfile);
        return 1;
    }

    int ret = avformat_open_input(&ctx, vfile, 0, 0);
    if ( ret < 0) {
        char buf[512];
    ...
    av_dump_format(ctx, 0, vfile, 0);
    // init video decoder
    AVCodecContext* dctx;
    int vst = open_decoder(&dctx, ctx);
    if (vst < 0)
        return 1;
    ...
    while ( av_read_frame(ctx, p) >= 0 ) {
        if (p->stream_index == vst) {
            ...
            if (vf >= 0) {
              ...
              //  fprintf(stderr, "Could not find matching video frame\n");
            }
        }
