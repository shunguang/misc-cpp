...
//----------------- global funcs ---------------------
void ngv::dumpLog( const std::string &x, ... )
{
	ngv::dumpLog( x.c_str() );
}

void ngv::dumpLog(const char *fmt, ...)
{
...
//-------------------------------
void ngv::startLogThread( const std::string &logFilePath, const bool showInConsole)
{
...
void  ngv::appExit(const int flag)
{
	if (flag != 0) {
...
void  ngv::appExit(const  std::string &x, ...)
{
	dumpLog(x);
	endLogThread();
	exit(1);
...
void ngv::appAssert( const bool flag, const std::string &msg)
{
	if (!flag) {
		dumpLog(msg);
		endLogThread();
...
void  ngv::appAssert(const bool flag, const char *file, const int lineNum)
{
	if (!flag) {
