...
//-------------------------------
void cali::startLogThread( const std::string &logFilePath, const bool showInConsole)
{
...
void  cali::appExit(const int flag)
{
	if (flag != 0) {
...
void  cali::appExit(const char * x, ...)
{
	dumpLog(x);
	endLogThread();
	exit(1);
...
void  cali::appExit(const  std::string &x, ...)
{
	dumpLog(x);
	endLogThread();
	exit(1);
...
void cali::appAssert( const bool flag, const std::string &msg)
{
	if (!flag) {
		dumpLog(msg);
		endLogThread();
...
void  cali::appAssert(const bool flag, const char *file, const int lineNum)
{
	if (!flag) {
