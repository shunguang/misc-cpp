...
//return a col vector
Mat cali::sumCols(const cv::Mat &x)
{
	//cout<<x<<endl;
	int n = x.cols;
...
//return a row vector
Mat cali::sumRows(const cv::Mat &x)
{
	//cout<<x<<endl;
...
void cali::gray2bgr(const cv::Mat &gray, cv::Mat &bgr)
{
	vector<Mat> ch;
	ch.push_back(gray);
	ch.push_back(gray);
...
void cali::convert_to_CV_8U(const cv::Mat &in, cv::Mat &out)
{
...
bool cali::myCvInv(const cv::Mat &X, cv::Mat &invX, double &det)
{
	det = determinant(X);
	if (det < APP_EPS_32F) {
		return false;
...
float cali::maskNonZeroPercent(const Mat &mask, const int &xo, const int &yo, const int &xb, const int &yb, size_t &n)
{
#if 1
...
void cali::rectPyrLevelUpDown(const cv::Rect &in, const int dL, cv::Rect &out)
{
	if (dL == 0) {
		out = in;
		return;
...
void cali::setCvImg(IplImage *des, const int &desRoiXo, const int &desRoiYo,
...
bool cali::getCvImgRowStep(const IplImage *img, int &rowStep, int &row0)
{
...
void cali::myImgWrite(const cv::Mat &x, const std::string &dir, const std::string &fname, const size_t fn, const size_t L)
{
	char buf[1024];
...
void cali::dumpCvImgToTextFile(const IplImage *J, const string &fileName)
{
	int w = J->width;
	int h = J->height;
...
void cali::drawPlus(cv::Mat &img, const cv::Point2f &pt, const uint32_t len, const cv::Scalar& color, int thickness, int lineType, int shift)
{
	Point p(pt.x, pt.y);
	drawPlus(img, p, len, color, thickness, lineType, shift);
}

void cali::drawPlus(Mat &img, const Point &pt, const uint32_t len, const Scalar& color, int thickness, int lineType, int shift)
{
...
void cali::drawPluses(cv::Mat &img, const std::vector<cv::Point2f> &v, const uint32_t len, const cv::Scalar& color, int thickness, int lineType, int shift)
{
...
void cali::drawPluses(cv::Mat &img, const std::vector<cv::Point> &v, const uint32_t len, const cv::Scalar& color, int thickness, int lineType, int shift)
{
...
void cali::wrtieStrings(Mat& img, const vector<string>& vStrs, Point &org, int fontFace, double fontScale, Scalar color, int thickness, int lineType)
{
