...
#include "libcalibIR/thermalvis_core/tools.hpp"
#include "libcalibIR/thermalvis_core/improc.hpp"
#include "libcalibIR/thermalvis_core/opencv_redefinitions.hpp"
...
#define MIN_CORNER_SEPARATION 2            //3 for 4x6 holes,2 for 2x3 holes
#define MAX_CORNER_SEPARATION 250          //250 (for 4x6 holes larger image for intrsic, since the board can only be see by one camera ) 
                                           //150 (for 4x6 holes smaller image for stereo, since the board need be seen by both cameras ) 
...
#define MINIMUM_MSER_AREA		30          //wMin = (imgW/13)/2, areaMin = wMin*Wmin
											//for lapton: wMin = 160/26 =  6
											//for boson: wMin = 640/26  =  24
...
bool findMaskCorners(const cv::Mat& image, cv::Size patternSize, vector<cv::Point2f>& corners, int detector = 0, int mserDelta = 8, double max_var = 0.25, double min_div = 0.2, double area_threshold = 1.01, double maxCornerDistInPixel=100);

/// \brief 		Core pattern-finding function
bool findPatternCorners(const cv::Mat& image, cv::Size patternSize, vector<cv::Point2f>& corners, int mode, int detector = 0, int mserDelta = 8, double max_var = 0.25, double min_div = 0.2, double area_threshold = 1.01, double maxCornerDistInPixel=100);
