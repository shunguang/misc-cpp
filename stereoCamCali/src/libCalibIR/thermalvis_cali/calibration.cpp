...
bool findMaskCorners(const cv::Mat& image, cv::Size patternSize, vector<cv::Point2f>& corners, int detector, int mserDelta, double max_var, 
	double min_diversity, double area_threshold, double maxCornerDistInPixel)
{
	cv::Mat grayIm;
    if (image.channels() > 1) cvtColor(image, grayIm, RGB2GRAY);
    else grayIm = cv::Mat(image);
    return findPatternCorners(grayIm, patternSize, corners, 1, detector, mserDelta, max_var, min_diversity, area_threshold, maxCornerDistInPixel);
}
...
bool findEdgePatches(cv::Size patternSize, int mode, int *XVec, int *YVec, vector<cv::Point2f>& patchCentres, vector<cv::Point2f>& remainingPatches)
{
...
bool findInteriorPatches(cv::Size patternSize, int mode, int *XVec, int *YVec, vector<cv::Point2f>& patchCentres, vector<cv::Point2f>& remainingPatches)
{
    // Temporary:
...
bool sortPatches(cv::Size imageSize, cv::Size patternSize, vector<cv::Point2f>& patchCentres, int mode)
{
    // TODO:
	...
	if (!findEdgePatches(patternSize, mode, XVec, YVec, patchCentres, remainingPatches)) {
		return false;
	}

    // Find (or simulate) interior patches
	if (!findInteriorPatches(patternSize, mode, XVec, YVec, patchCentres, remainingPatches)) {
		return false;
	}
...
void debugDisplayPattern(const cv::Mat& image, cv::Size patternSize, cv::Mat& corners, bool mode, double delay) {
    ...
    (image.channels() > 1) ? cvtColor(image, dispMat, RGB2GRAY) : image.copyTo(dispMat);
...
bool findPatternCorners(const cv::Mat& image, cv::Size patternSize, vector<cv::Point2f>& corners, int mode, int detector, int mserDelta, double max_var, 
	double min_div, double area_threshold, double maxCornerDistInPixel)
{
	...
	vector<vector<cv::Point> > msers2;
	for (int i = 0; i < msers.size(); ++i) {
		if (msers[i].size() < 4) {
			continue;
		}
		msers2.push_back(msers[i]);
	}
	msers = msers2;

    cout << "GAMMA=" << DEBUG_MODE << endl;
    if (DEBUG_MODE > 2){
		
		cv::imshow("mainWin", image);
		cv::waitKey(0);
	...
	//added by swu:
	//check if there has any nan point in <patchCentres2f>
	assert(msers.size() == patchCentres2f.size());
	vector<cv::Point2f> patchCentres2f_out;
	vector<vector<cv::Point> > msers_out;
	for (int i = 0; i < patchCentres2f.size(); ++i) {
		cv::Point2f &p0 = patchCentres2f[i];
		if (isnan(p0.x) || isnan(p0.y)) {
			continue;
		}

		patchCentres2f_out.push_back(p0);
		msers_out.push_back(msers[i]);
	}

	if (patchCentres2f_out.size() < desiredPatchQuantity) {
		printf("%s << Insufficient patches found. Returning.---BB \n", __FUNCTION__);
		return false;
	}

	patchCentres2f = patchCentres2f_out;
	msers = msers_out;
    if (DEBUG_MODE > 1){
	...
	if (!sortPatches(image.size(), patternSize, patchCentres2f, mode)) {
		return false;
	}
    ...
    found = verifyPatches(image.size(), patternSize, patchCentres2f, mode, MIN_CORNER_SEPARATION, maxCornerDistInPixel);
    if (!found){
        if (DEBUG_MODE > 1){
    ...
    correctPatchCentres(image, patternSize, patchCentres2f, mode, maxCornerDistInPixel);
...
bool correctPatchCentres(const cv::Mat& image, cv::Size patternSize, vector<cv::Point2f>& patchCentres, int mode, double maxCornerDistInPixel)
{
    ...
    found = findPatternCentres(undistortedImage, patternSize, newCentres, mode, maxCornerDistInPixel);
    ...
    found = findPatternCentres(enhancedCol, patternSize, newCentres, 0,maxCornerDistInPixel);
...
bool findPatternCentres(const cv::Mat& image, cv::Size patternSize, vector<cv::Point2f>& centres, int mode, double maxCornerDistInPixel)
{
    // mode 0: cv::MSER chessboard finder
    // mode 1: cv::MSER mask finder
	...
	if (!sortPatches(image.size(), patternSize, centres, mode)) {
		return false;
	}
    ...
    found = verifyPatches(image.size(), patternSize, centres, mode, MIN_CORNER_SEPARATION, maxCornerDistInPixel);
...
bool verifyPattern(cv::Size imSize, cv::Size patternSize, vector<cv::Point2f>& patternPoints, double minDist, double maxDist)
{
	...
	double factorThd = 2.0;                      
	if (imSize.width == 160) { //lepton
		factorThd = 4.10;
	}
        ...
        for (int j = 0; j < patternSize.width-2; j++)
        {
            ...
            */
            ...
            if (dist > maxStraightnessDist)
            {
                if (DEBUG_MODE > 0)
                {
					dumpLog("%s << Row out of alignment, dist=%f, maxStraightnessDist=%f", __FUNCTION__,dist, maxStraightnessDist);
                }
            ...
            if (factor > factorThd)
            {
                if (DEBUG_MODE > 0)
                {
					dumpLog("%s << Row factor is out.\n", __FUNCTION__);
					dumpLog("%s << factor / [dist1, dist2] = %f / [%f, %f].\n", __FUNCTION__, factor, dist1, dist2);
                }
                //cv::waitKey(0);
                return false;
            }
    ...
    for (int i = 0; i < patternSize.height-2; i++)
    {
        for (int j = 0; j < patternSize.width; j++)
        {
            ...
            if (dist > maxStraightnessDist)
            {
                if (DEBUG_MODE > 0)
                {
					dumpLog("%s << Column out of alignment, dist=%f, maxStraightnessDist=%f\n", __FUNCTION__, dist, maxStraightnessDist);
                }
                //cv::waitKey(0);
                return false;
            }
            ...
            if (factor > factorThd)
            {
                if (DEBUG_MODE > 0)
                {
					dumpLog("%s << Column out of scale, factor=%f \n", __FUNCTION__, factor);
                }
                //cv::waitKey(0);
                return false;
            }
            ...
            // for (int a = max(0,i-1); a < min(patternSize.height,i+1+1); a++) {
				// for (int b = max(0,j-1); b < min(patternSize.width,j+1+1); b++) {
					...
					if ((dist > maxDist) && (a >= i-1) && (a <= i+1) && (b >= j-1) && (b <= j+1)) {
						if (DEBUG_MODE > 0)	{
							dumpLog("%s << cv::Point (%d, %d) failed adjacent test (%f) [%d, %d] vs [%f, %f].\n", __FUNCTION__, i, j, dist, a, b, minDist, maxDist);
						}
						//cv::waitKey(0);
						return false;
					}
					...
					if (dist <= minDist) {
						...
						if (DEBUG_MODE > 0)	{
							dumpLog("%s << cv::Point (%d, %d) failed closeness test (%f) [%d, %d] vs [%f, %f].\n", __FUNCTION__, i, j, dist, a, b, minDist, maxDist);
						}
						//cv::waitKey(0);
						return false;
					...
					if ((DEBUG_MODE > 0) && (dist < 1.0))	{
						dumpLog("%s << cv::Point (%d, %d) has distance of (%f) to [%d, %d] vs [%f, %f].\n", __FUNCTION__, i, j, dist, a, b, minDist, maxDist);
					}
				}
