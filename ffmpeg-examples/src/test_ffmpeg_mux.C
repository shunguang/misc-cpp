...
static int initialize_stream(struct OutputStream *os, char **argv)
{
    ...
    printf("initialize_stream(): filename=%s,encoder=%s,w=%d,h=%d,spixfmt=%s,tbn=%d,tbd=%d, bitrate=%ld", filename, encoder, w, h, spixfmt, tbn, tbd, bitrate);
...
{
	if (argc!=2){
		printf("usage: test_ffmepg_mux encodeerCodecName\n");
		printf("note:\n"); 
		printf("  (1) $ffmpeg -encoders | grep h264 to find the supported encoders in your machine\n");
		printf("  (2) some codec name list: [h264_nvenc, nvenc, h264_nvmpi,h264_v4l2m2m; cpu based: libx264,libx264rgb]\n");
		return -1;
	}


	const char *encCodecName = argvIn[1];
    ...
    cfg.tbn= 1001;  //timebase numerorator
    cfg.tbd = 30000; //timebase denominator
    cfg.bitrate=10000000;

    cfg.ffmpegEncName = std::string(encCodecName);
    cfg.inputKlvFile = "./klv.bin";
    cfg.inputYuvFile = "./vid.bin";
    cfg.ouputFilePath = ".";
    cfg.useVidCapFilterAfterEncoder=false;
    cfg.nFrmsToPeekCpuTimeProfile=100;
