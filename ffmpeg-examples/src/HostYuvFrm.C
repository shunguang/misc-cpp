...
{
#if DEBUG_
	printf("HostYuvFrm::HostYuvFrm(): size does not match!\n");
...
void HostYuvFrm::creatBuf()
{
	if( sz_ > 0 ){
		...
		if (!buf_) {
			printf("HostYuvFrm::creatBuf(): cannot allocate memory!\n");
			assert(0);
		}
	}
	else {
...
void HostYuvFrm::hdCopyToBgr(cv::Mat &picBGR) 
{
	...
	cv::cvtColor(picYV12, picBGR, cv::COLOR_YUV420p2RGB);
}
...
void HostYuvFrm::wrtFrmNumOnImg()
{
	...
	uint64_t dt = 0; //TS_SINCE_EPOCH_MS - ts_;
	cv::Mat picYV12 = cv::Mat(h_ * 3 / 2, w_, CV_8UC1, buf_);
	cv::cvtColor(picYV12, bgr, cv::COLOR_YUV420p2RGB);

	cv::putText( bgr, (my_to_string(fn_) + ", [" + my_to_string(w_) + "x" + my_to_string(h_) + "]"), 
...
{
	assert(w_ == bgr.cols && h_ == bgr.rows);
